...
**Latest source files update: [June 2, 2020]** Updated OpenCore to 0.5.9, enabled devices to request extra USB current to charge iPads or similar devices by adding proper USBX device according to SMBIOS via ACPI (SSDT-USBX.aml),removed apfs.efi (using OpenRuntime builtin APFS),removed SSDT-HPET.aml,renamed SSDT-DDGPU.aml to SSDT-dDGPU.aml, using SSDT-PNLFCFL.aml (new pnlf acpi for CFL).
